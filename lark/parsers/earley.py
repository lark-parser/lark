        ...
        for item in items:
            ...
            if item.is_complete:
                ...
                if is_terminal(item.expect):
                    ...
                    # XXX: Probably a hack:
                    # The code above to detect ambiguous matches only notices and updates
                    # the _2nd_ of an ambiguous pair. This is because new Items for the 1st are
                    # generated by complete() _after_ we added it to to_reduce() above, and
                    # we don't have a reference to make the trees on those new items _ambig.
                    #
                    # So we always want to use the latter set of Items we receive for duplicates,
                    # to ensure they are marked _ambig where required. 
                    # TODO: moving to SPPF forests or building the new items differently would
                    # fix this, but requires significant changes with dependencies on other
                    # parsers.
                    # 
                    if k in self.predicted:
                        self.predicted.remove(k)
                        self.to_predict.remove(item)
