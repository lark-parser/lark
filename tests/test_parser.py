    ...
    class _TestFullEarley(unittest.TestCase):
        ...
        def test_ambiguous_intermediate_node(self):
            ...
            expected = {
                Tree(
                    "start",
                    [Tree("ab", ["A"]), Tree("bc", ["B", "C"]), Tree("d", ["D"])],
                ),
                Tree(
                    "start",
                    [Tree("ab", ["A", "B"]), Tree("bc", ["C"]), Tree("d", ["D"])],
                ),
            }
            self.assertEqual(ambig_tree.data, "_ambig")
        ...
        def test_ambiguous_symbol_and_intermediate_nodes(self):
            ...
            expected = {
                Tree(
                    "start",
                    [Tree("ab", ["A", "B"]), Tree("bc", ["C"]), Tree("cd", ["D"])],
                ),
                Tree(
                    "start",
                    [Tree("ab", ["A", "B"]), Tree("bc", []), Tree("cd", ["C", "D"])],
                ),
                Tree(
                    "start",
                    [Tree("ab", ["A"]), Tree("bc", ["B", "C"]), Tree("cd", ["D"])],
                ),
                Tree(
                    "start",
                    [Tree("ab", ["A"]), Tree("bc", ["B"]), Tree("cd", ["C", "D"])],
                ),
            }
            self.assertEqual(ambig_tree.data, "_ambig")
            self.assertEqual(set(ambig_tree.children), expected)
...
def _make_parser_test(LEXER, PARSER):
    ...
    class _TestParser(unittest.TestCase):
        ...
        @unittest.skipIf(
            sys.version_info[0] == 2 or sys.version_info[:2] == (3, 4),
        ...
        def test_bytes_utf8(self):
            ...
            for enc, j in [
                ("sjis", u"地球の絵はグリーンでグッド?  Chikyuu no e wa guriin de guddo"),
                ("sjis", u"売春婦"),
                ("euc-jp", u"乂鵬鵠"),
            ]:
                s = j.encode(enc)
        ...
        @unittest.skipIf(
            PARSER != "earley" or LEXER == "standard",
        ...
        @unittest.skipIf(
            not regex or sys.version_info[0] == 2,
            "Unicode and Python 2 do not place nicely together.",
        )
