...
EXTRACT_STANDALONE_MODULES = set()
...
class _Unpickler(pickle.Unpickler):
    def find_class(self, module_name, global_name):
        if module_name in EXTRACT_STANDALONE_MODULES:
...
def gen_standalone(lark_inst, output=None, out=sys.stdout, compress=False):
    ...
    def output_decompress(name):
        output('%(name)s = loads(zlib.decompress(base64.b64decode(%(name)s)))' % locals())

    def output_extract_standalone_modules():
        output("EXTRACT_STANDALONE_MODULES = " + repr(files_to_modules(EXTRACT_STANDALONE_FILES)))
    ...
    for i, pyfile in enumerate(EXTRACT_STANDALONE_FILES):
        with open(os.path.join(_larkdir, pyfile)) as f:
            ...
            code = sections['standalone']
            if i:   # if not this file
                code = strip_docstrings(partial(next, iter(code.splitlines(True))))
            output(code)
            if compress and (compressed := sections.get("compressed")):
                output(compressed)
            

    data, m = lark_inst.memo_serialize([TerminalDef, Rule])
    if compress:
        output('import zlib, base64')
        output_extract_standalone_modules()
        output('DATA = (')
